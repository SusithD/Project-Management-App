...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log("[JIRA API] Debug info:", {
      ...
      emailEndsWithDemo: userEmail ? userEmail.includes("@demo.com") : false,
      nodeEnv: process.env.NODE_ENV,
      explicitDemoMode: process.env.DEMO_MODE,
    });

    // Always return demo data if in development mode and user email contains @demo.com
    if (
      (demoMode && isDemoUser) ||
      (process.env.NODE_ENV === "development" &&
        userEmail &&
        userEmail.includes("@demo.com"))
    ) {
