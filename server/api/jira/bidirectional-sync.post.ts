...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (demoMode && isDemoUser) {
      console.log("[JIRA API] Demo mode: Simulating bi-directional sync...");

      // Return a mock successful sync response
      return {
        statusCode: 200,
        body: {
          success: true,
          message: "Bi-directional sync completed successfully (Demo Mode)",
          conflictsDetected: [],
          conflictsResolved: [],
    ...
    console.log(
      `[JIRA API] Starting bi-directional sync for project ${projectId} and issue ${jiraIssueKey}`,
    );
    ...
    const projectLastUpdated =
      project.lastUpdated || project.createdAt || new Date().toISOString();
    ...
    console.log(
      `[JIRA API] Bi-directional sync completed for project ${project.name}`,
    );
      ...
      body: {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to perform bi-directional sync",
      },
    };
  }
});
