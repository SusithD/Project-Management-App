...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log(
      `[JIRA API] Loading issues for project: ${projectKey} (max: ${maxResults})`,
    );
    ...
    console.log("[JIRA API] Debug info:", {
      demoMode,
      userEmail,
      isDemoUser,
      projectKey,
      hasIssuesForProject: !!DEMO_JIRA_ISSUES[projectKey],
    });

    if (demoMode && isDemoUser) {
      console.log(
        `[JIRA API] Returning demo issues for project: ${projectKey}`,
      );
    ...
    console.log(
      `[JIRA API] Found ${issues.length} issues for project ${projectKey}`,
    );
